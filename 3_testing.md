...
- **Frontend:** Trang lọc phim, Dashboard, Quản lý tin tức, Công cụ xếp lịch
- **Testing:** Test Case TC01-TC04


**23120047 - Nguyễn Gia Huy - Contribution (25%)**
- **Backend:** API lịch chiếu, API thông tin phim, Kết nối API với Frontend
- **Testing:** Test Case TC12-TC15


**23120049 - Nguyễn Thanh Huyền - Contribution (25%)**
- **Frontend:** Chatbot Widget, Quản lý phim, Quản lý phòng ghế, Quản lý giao dịch
- **Testing:** Test Case TC05-TC08


**23120060 - Trần Kim Ngân - Contribution (25%)**
- **Backend:** API thanh toán, API quản lý tin tức, API chỗ ngồi, Kết nối API với Frontend
- **Testing:** Viết Test Plan, Test Case TC09-TC11


<p align="center">
  <img 
    src="data/image_template_3/jira.png"
...
* **Đối tượng kiểm thử (Testing Objects):**
  * **Về mặt Chức năng:** Kiểm thử toàn bộ 5 phân hệ tính năng cốt lõi của hệ thống bao gồm: Quản lý tài khoản (Đăng ký, đăng nhập, phân quyền); Quản lý phim và suất chiếu phía Admin; Tìm kiếm & Bộ lọc phim; Đặt ghế trực quan theo thời gian thực; Thanh toán tích hợp cổng VNPAY và Xuất vé điện tử (ETicket) chứa mã QR.
...
3. **Tìm kiếm & Bộ lọc (Search & Dynamic Filter):** Khách hàng tìm phim và lọc phim theo thể loại, tình trạng.
...
5. **Thanh toán & Xuất vé (Payment & Ticketing):** Xử lý giao dịch qua cổng VNPAY, cập nhật trạng thái hóa đơn, quản lý thời gian giữ ghế và xuất vé điện tử chứa mã QR.
...
| **TC12** | Khách hàng thanh toán đặt vé thành công qua cổng VNPAY | Thanh toán & Xuất vé | Luồng đúng: Trừ tiền chính xác, chuyển trạng thái đơn hàng sang "Đã thanh toán". |
| **TC13** | Khách hàng chủ động hủy giao dịch tại trang thanh toán của bên thứ ba | Thanh toán & Xuất vé | Luồng ngoại lệ: User bấm "Quay lại" tại trang VNPAY, hệ thống hoàn tác ghế trống. |
...
![TC01](data/image_template_3/3.1.4.png)
---
...
### 3.2.9 TC09 - Chọn ghế trống thành công trên sơ đồ phòng chiếu
...
---
### 3.2.10 TC10 - Khóa ghế real-time khi có user khác đang nhanh tay giữ trước
...
---
### 3.2.11 TC11 - Hủy chọn ghế đã chọn (Deselect) thành công
...
Video thuyết trình: [LINK](https://youtu.be/LapalHNtgxA)
